...
- Product and output versions are split: Streamlit workspace `RESULT_VERSION` is `4.6.5`, OCR full JSON is `4.6.0`, and native JSON is `5.0.0`/`5.1.0`. Compatibility ownership is manual.
...
- Streamlit configures package logging and the launcher follows local service logs, but the project has no external aggregation, metrics, tracing, or alerting backend. Operational history remains split across local files and result models.
