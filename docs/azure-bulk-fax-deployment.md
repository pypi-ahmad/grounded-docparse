...
The repository is **not deployable in this form as a shared PHI service**. Its supported runtime is a trusted single-user workstation, the UI processes up to 20 files sequentially in session state, reusable definitions use local SQLite, and it has no durable jobs, multi-user authorization, or tenant isolation. Complete the application changes and acceptance gates in this document before exposing the service. Do not publish ports `9356`, `8080`, `8118`, or `8119` from the current launch scripts.
