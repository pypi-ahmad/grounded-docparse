...
try {
  ...
  const fileNodes = graph.nodes.filter(n => fileTypes.has(n.type)).map(n => n.id), assigned = new Map();
  ...
  graph.layers.forEach(layer => (layer.nodeIds || []).forEach(id => {
  ...
  const count = (items, key) => items.reduce((a, item) => { a[item[key]] = (a[item[key]] || 0) + 1; return a; }, {});
  const stats = { totalNodes: graph.nodes.length, totalEdges: graph.edges.length, totalLayers: graph.layers.length, tourSteps: graph.tour.length, nodeTypes: count(graph.nodes, 'type'), edgeTypes: count(graph.edges, 'type') };
